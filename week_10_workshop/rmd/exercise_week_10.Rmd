---
title: "Workshop 10: ggplot scales, themes, and color palettes"
output:
...
-   **Annotate data points with `geom_label()`.** Add a text annotations
...
-   **Compound plot with total counts.** Create a smaller plot that
    shows the total counts across all four cantons, similar to the small
    light blue plots under the main plots on the [epigraphhub
    dashboard](https://epigraphhub.org/superset/dashboard/11). Make a
...
-   **Rolling averages.** The current graph shows daily counts directly
    from the data. We can also plotting the 7-day rolling averages to
    smooth out the bumpiness in the daily counts. This is ofter the
    format used in time-series plots of epidemic dat. Plot a weekly
    rolling average on your graph by following this
