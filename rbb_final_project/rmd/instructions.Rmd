...
A typical approach for the report structure is to select a principal variable, then generate plots illustrating how that variable's distribution shifts in relation to other variables. For instance, one could analyze how contraceptive use varies among different segments of the population. Alternatively, you might examine how access to clean water differs across various socio-economic groups. There are hundreds of variables to choose from!

To submit, you should

-   upload a zip folder with two files---your Rmarkdown and your knitted HTML/PDF/DOCX file---to the assignment page.

Those are the instructions in a nutshell. 

The rest of this document consists of recommended analysis steps and a set of optional appendices.

# **Step 1: Obtain the women's recode file for a specific country**

DHS data is normally obtained by request from the DHS website [here](https://dhsprogram.com/data/new-user-registration.cfm){target="_blank"}.
...
# **Step 3: Identify and analyze the relevant variables**
...
Sometimes the variable descriptions in the .MAP files are not detailed enough. In that case, you can search for the variable name in this DHS recode [manual](https://dhsprogram.com/pubs/pdf/DHSG4/Recode7_DHS_10Sep2018_DHSG4.pdf){target="_blank"}.

The recode manuals are similar to the MAP files, but they explain each variable in a bit more detail.
...
Note that the variables covered by the DHS vary from survey to survey, so you may need some trial and error to find an interesting topic that is covered in the dataset you chose.

Now that you have the datasets and variables you want to work with, you can do a quick read through the sample reports below and the grading rubric to guide your analysis.
...
-   Zip the Rmd and the knitted output into a single folder, and upload to the assignment page.
...
Demonstrate that you can use R Markdown to create a professional document. This includes using appropriate formatting, including headings and lists, and using Rmarkdown features such as inline code.
...
# **Appendix 3: Information about the DHS**
